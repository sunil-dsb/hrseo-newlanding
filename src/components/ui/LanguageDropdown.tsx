...
interface LanguageDropdownProps {
  className?: string;
  variant?: "navbar" | "footer" | "dashboard";
  align?: "left" | "right";
}
      ...
      <div ref={dropdownRef} className={`relative ${className}`}>
        ...
        {isOpen && (
          <div
            ...
            } top-full mt-4 w-48 bg-white border border-black/4 shadow-xl rounded-lg p-1 z-50 overflow-hidden ring-1 ring-black/3`}
            role="menu"
          ...
          >
            {languages.map((language) => (
              <button
                ...
                role="menuitem"
                aria-current={
                  currentLang.code === language.code ? "true" : "false"
                }
              >
      ...
      <div ref={dropdownRef} className={`relative ${className}`}>
        ...
        {isOpen && (
          <div
            className={`absolute ${
              align === "left" ? "left-0" : "right-0"
            } top-full mt-2 w-48 bg-white border border-zinc-200 shadow-xl rounded-xl p-1 z-50 overflow-hidden`}
            role="menu"
            aria-label="Language options"
          >
            {languages.map((language) => (
              <button
                key={language.code}
                onClick={() => handleLanguageChange(language)}
                className="w-full flex items-center gap-3 px-3 py-2 text-left transition-colors text-zinc-600 hover:text-brand-primary hover:bg-brand-light/20 rounded-lg"
                role="menuitem"
                aria-current={
                  currentLang.code === language.code ? "true" : "false"
                }
              >
                <language.Flag className="shrink-0 scale-75" />
                <span
                  className={`text-sm transition-all ${
                    currentLang.code === language.code
    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      {isOpen && (
        <nav
          className={`absolute ${
            align === "left" ? "left-0" : "right-0"
          } bottom-full mb-1 w-40 bg-white border border-zinc-200 z-20 shadow-lg rounded-lg p-1`}
